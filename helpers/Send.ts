...
import { BigNumber, JsonRpcProvider, Wallet, parseEther } from "./Ethers";
...
//     if (response.data && response.data.result) {
//       console.log(
//         "Matic sent successfully. Transaction hash:",
...
export async function sendMatic(
	...
	const transactionData = {
	  from: senderAddress,
	  to: receiverAddress,
	  value: BigNumber.from(amount).mul(parseEther("0.001")).toHexString(),
	  gas: "0x5208", // Gas limit
	  gasPrice: await provider.getGasPrice(), // Gas price (10 Gwei)
	  chainId: 80001, // Chain ID (Matic Mumbai testnet)
	};
	...
	try {
	  ...
	  if (response) {
		...
		console.error("Failed to send Matic");
	  }
	} catch (error) {
	  console.error("Error sending Matic:", error);
	}
